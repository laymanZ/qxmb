package com.example.z.faMenu_Button;
...
public class FloatingActionsMenu extends ViewGroup {
  ...
  /*show 和 getMarginRight 和 isShown 是自己添加的方法*/
  private boolean isShown = true;
  private int ANIM_DURATION = 200;
  private boolean mVisible = false;
  public void show(boolean isVisible) {
    mVisible = isVisible;
    int translationY = isVisible ? 0 : (getWidth()/2) + getMarginTop();
    this.animate().translationY(translationY).setDuration(ANIM_DURATION).start();
  }
  ...
  private int getMarginTop() {
    ...
    if (layoutParams instanceof ViewGroup.MarginLayoutParams) {
      marginBottom = ((MarginLayoutParams) layoutParams).topMargin;
    }
    return marginBottom;
  }
